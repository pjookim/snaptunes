...
// Spotify 토큰 관련 함수들
function saveSpotifyTokens(accessToken: string, refreshToken: string, expiresAt: number) {
  if (typeof window === "undefined") return;
  try {
    const tokens = {
      accessToken,
...
// 토큰 갱신 함수
async function refreshSpotifyToken(refreshToken: string): Promise<{ accessToken: string; refreshToken: string; expiresIn: number } | null> {
  try {
...
async function getValidSpotifyToken(setTokenCallback?: (token: string | null) => void): Promise<string | null> {
  ...
  if (refreshed) {
    ...
    saveSpotifyTokens(refreshed.accessToken, refreshed.refreshToken, newExpiresAt);
    if (setTokenCallback) {
...
// Spotify 사용자 정보 가져오기
async function getSpotifyUserInfo(accessToken: string): Promise<{ id: string; displayName: string; email: string; imageUrl?: string } | null> {
  try {
...
export default function Home() {
  ...
  const [spotifyUser, setSpotifyUser] = useState<{ id: string; displayName: string; email: string; imageUrl?: string } | null>(null);
  ...
  const stepCards = [
      ...
      content: (
        <>
          <div className="flex items-center justify-between mb-2">
            <span className="font-bold text-xl tracking-wider">{t('steps.step1.title')}</span>
            {spotifyToken && <span className="text-green-700 font-bold">{t('steps.step1.done')}</span>}
          </div>
          <p className="text-base text-neutral-700 mb-4 font-mono">{t('steps.step1.description')}</p>
          <Button onClick={handleSpotifyAuth} disabled={!!spotifyToken} variant="neutral">
            {spotifyToken ? t('steps.step1.buttonAuthenticated') : t('steps.step1.button')}
          </Button>
        </>
      ),
      minHeight: 260
    },
    {
      ...
      content: (
        <>
          <div className="flex items-center justify-between mb-2">
            <span className="font-bold text-xl tracking-wider text-black">{t('steps.step2.title')}</span>
            {ocrResult.length > 0 && <span className="text-green-700 font-bold">{t('steps.step2.done')}</span>}
          </div>
          <p className="text-base text-neutral-700 mb-4 font-mono">{t('steps.step2.description')}</p>
          <div className="mb-4">
          ...
          >
            {isLoading && step === 2 ? t('steps.step2.loading') : t('steps.step2.button')}
          </Button>
          {ocrResult.length > 0 && (
            <>
    ...
    },
    {
      ...
      content: (
        <>
          <div className="flex items-center justify-between mb-2">
            <span className="font-bold text-xl tracking-wider">{t('steps.step3.title')}</span>
            {spotifyTracks.length > 0 && <span className="text-green-700 font-bold">{t('steps.step3.done')}</span>}
          </div>
          <p className="text-base text-neutral-700 mb-4 font-mono">{t('steps.step3.description')}</p>
              ...
              <ul className="mt-4 space-y-2">
                {spotifyTracks.map((track, idx) => (
                  <li key={idx} className="flex items-center gap-2 text-base w-full">
                    {/* 앨범 커버 */}
                    {track.albumArt && (
                      <img src={track.albumArt} alt={t('common.album')} className="w-10 h-10 rounded border-2 border-black shrink-0" />
                    )}
                    {/* 노래 정보 */}
                    ...
                    {track.found ? (
                      ...
                      <span className="ml-auto text-red-500 font-bold">{t('steps.step3.notFound')}</span>
                    )}
                  </li>
                ))}
              </ul>
    ...
    },
    {
      ...
      content: (
        <>
          <div className="flex items-center justify-between mb-2">
            <span className="font-bold text-xl tracking-wider">{t('steps.step4.title')}</span>
            {playlistUrl && <span className="text-green-700 font-bold">{t('steps.step4.done')}</span>}
          </div>
          <p className="text-base text-neutral-700 mb-4 font-mono">{t('steps.step4.description')}</p>
          <Input
          ...
          <Button
            ...
            className="w-full font-bold border-2 border-black shadow-[3px_3px_0_0_#222] bg-white text-black hover:bg-black hover:text-white transition"
          >
            {isLoading && step === 4 ? t('steps.step4.creating') : t('steps.step4.button')}
          </Button>
          {playlistUrl && (
            <div className="mt-4 flex flex-col items-center gap-2">
              ...
              <div className="text-sm text-neutral-700 text-center">
                {t('steps.step4.by')} {playlistMeta?.ownerUrl ? (
  ...
  useEffect(() => {
    const initializeSpotifyToken = async () => {
      ...
      const urlRefreshToken = new URL(window.location.href).searchParams.get("spotify_refresh_token");
  ...
  }, [step, text, ocrResult, spotifyTracks, selectedTrackIds, playlistName, isExtracted, isSearched, spotifyUser]);
  ...
  useEffect(() => {
    ...
    if (savedState && savedState.timestamp) {
      ...
      if (Date.now() - savedState.timestamp < oneHour) {
        ...
        if (savedState.selectedTrackIds) setSelectedTrackIds(savedState.selectedTrackIds);
  ...
  async function handleExtractSongs(overrideText?: string) {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : t('errors.extractionFailed'));
    } finally {
      setIsLoading(false);
    }
  }
  ...
  async function handleSearchSpotify() {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : t('errors.searchFailed'));
    } finally {
      setIsLoading(false);
    }
  }
  ...
  async function handleCreatePlaylist() {
    ...
    setPlaylistMeta(null);
    try {
      const meta = await createSpotifyPlaylist(foundTracks, validToken, playlistName || t('defaults.playlistName'));
      if (meta) {
        setPlaylistUrl(meta.playlistUrl);
        setPlaylistMeta(meta);
      }
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t('errors.createPlaylistFailed'));
    } finally {
      setIsLoading(false);
    }
  }
    ...
    <main className="flex min-h-screen flex-col items-center justify-start py-4 px-4">
      ...
      <div className="w-full max-w-2xl">
        <div className="flex flex-col items-center my-8 max-w-32 mx-auto md:max-w-md">
          <Image src={isDark ? "/snaptunes_logo_dark.svg" : "/snaptunes_logo.svg"} alt={t('app.logo')} className="mb-2" width={200} height={200} priority />
        </div>
            ...
            <div className="relative z-30 p-8" ref={contentRef}>
              {(() => {
                if (contentIdx === 0) return (
                  <>
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-bold text-xl tracking-wider text-black">{t('steps.step1.title')}</span>
                      {spotifyToken && <span className="text-green-700 font-bold">{t('steps.step1.done')}</span>}
                    </div>
                    <p className="text-base text-neutral-700 mb-4 font-mono">{t('steps.step1.description')}</p>
                    {spotifyToken && spotifyUser ? (
                      <div className="relative">
                        <div className="flex flex-row items-stretch bg-white border-2 border-black rounded-base shadow-[2px_2px_0_0_#222]">
                          ...
                          <div className="flex-1 p-4 flex flex-col justify-center">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex-1">
                                <div className="font-bold text-lg text-black">{spotifyUser.displayName}</div>
                          ...
                          <div className="flex flex-col items-center justify-center px-4 border-l-2 border-black min-h-full">
                            <Button variant="neutral" size="icon" aria-label={t('steps.step1.logout')} onClick={handleSpotifyLogout}>
                      ...
                      <Button onClick={handleSpotifyAuth} variant="neutral" className="w-full">
                  ...
                  <>
                    ...
                    {isLoading && step === 2 ? (
                      <div className="w-full">
                        ...
                        <div className="text-center text-xs font-bold mt-1 text-black" style={{ textShadow: '1px 1px 0 #fff' }}>{t('common.loading')}</div>
                      </div>
                    ) : (
                      <Button
                        variant="neutral"
                ...
                if (contentIdx === 2) return (
                  <>
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-bold text-xl tracking-wider text-black">{t('steps.step3.title')}</span>
                      {spotifyTracks.length > 0 && <span className="text-green-700 font-bold">{t('steps.step3.done')}</span>}
                    </div>
                    <p className="text-base text-neutral-700 mb-4 font-mono">{t('steps.step3.description')}</p>
                    {spotifyTracks.length > 0 && (
                ...
                if (contentIdx === 3) return (
                  <>
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-bold text-xl tracking-wider text-black">{t('steps.step4.title')}</span>
                      {playlistUrl && <span className="text-green-700 font-bold">{t('steps.step4.done')}</span>}
                    </div>
                    <p className="text-base text-neutral-700 mb-4 font-mono">{t('steps.step4.description')}</p>
                    <Input
                    ...
                    {playlistUrl && (
                      <div className="mt-4 flex flex-col items-center gap-2">
                        {playlistMeta?.cover && (
                          <a href={playlistMeta.playlistUrl || playlistUrl} target="_blank" rel="noopener noreferrer">
                            <img src={playlistMeta.cover} alt={t('common.playlistCover')} className="w-32 h-32 rounded shadow border-2 border-black mb-2" />
                          </a>
                        )}
                        <div className="text-lg font-bold text-center">
                          <a href={playlistMeta?.playlistUrl || playlistUrl} target="_blank" rel="noopener noreferrer" className="hover:underline">
