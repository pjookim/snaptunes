...
export async function POST(req: NextRequest) {
  ...
  if (!accessToken || !Array.isArray(tracks) || !playlistName) {
    console.error('Invalid request data:', { accessToken: !!accessToken, tracks: tracks?.length, playlistName })
  ...
  try {
    ...
    console.log('Creating Apple Music playlist:', { playlistName, tracksCount: tracks.length })
    ...
    if (!playlistDetailRes.ok) {
      ...
      console.error('Playlist detail fetch failed:', playlistDetailRes.status, errorText)
      return NextResponse.json(
    ...
    if (!playlistData) {
      console.error('No playlist data found in detail response:', playlistDetail)
      return NextResponse.json(
    ...
    return NextResponse.json(
      { error: '플레이리스트 생성 중 오류가 발생했습니다: ' + (error instanceof Error ? error.message : '알 수 없는 오류') },
      { status: 500 },
    )
  }
} 